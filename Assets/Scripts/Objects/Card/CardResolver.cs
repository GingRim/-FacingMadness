using UnityEngine;
    ...
    public class CardResolver
    {
        ...
        /// <param name="useMode">선택한 사용 코스트</param>
        /// <returns>사용 성공 여부</returns>
        public bool Use(CardData card, CharacterBase user, CharacterBase target, CardUseCost useMode)
        {
            // 카드 또는 사용자가 없으면 실패
            ...
            switch (card.color)
            {
                ...
                case CardColorType.Red:
                    //ResolveRed(card, user, target, useMode);
                    break;
        ...
        /// <param name="mode">선택한 사용 방식</param>
        /// <returns>지불 성공 여부</returns>
        private bool TryPayCost(CharacterBase user, CardUseCost mode)
        {
            // 사용자의 코스트 모듈 가져오기
